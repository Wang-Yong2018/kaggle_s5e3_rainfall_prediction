...
v3_rcp <- 
  ...
  step_mutate(
    ...
    #homidity = (humidity/100) 
    #cloud_ss = cloud/(cloud+sunshine),
    #est_temp = temparature / (0.5*(maxtemp + mintemp)),
    #dewpoint_max = dewpoint - maxtemp,
    #dewpoint_min = dewpoint - mintemp,
    dewpoint_averge = dewpoint -temparature,
    #ss2 = sunshine**2,
    #cloud_humidity = cloud * humidity,
    humidity = 1 / (1 + exp(-0.3*(humidity - 80)))
    #dewpoint_max = dewpoint -maxtemp
    #temp_range =maxtemp-mintemp 
    )|>
  #   step_bs(cloud, sunshine,  # 或指定变量名（如cloud, sunshine）
  #   #num_breaks = 4#,            # 分箱数量（如3箱）
  #   #options = list(cuts = "quantile")
  # )|>
...
# fit_eng_glm <- lr_w_fit|>extract_workflow('v3_glm') |>last_fit(df_split) |> extract_fit_engine()
# fit_eng_glm |>glance()
# fit_eng_glm |>tidy()
```
